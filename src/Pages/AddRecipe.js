  ...
  return (
    ...
    <div className="p-3">
      <p className="mb-6 text-lg text-gray-700">
        We're delighted to have you on board. To get started, please add all the
        dishes from your menu along with the ingredients and other costs
        required to prepare each dish.
      </p>

      <p className="mb-6 text-lg text-gray-700">
        <strong className="text-red-500">Heads up:</strong> For each dish
        specify whether the quantity of ingredients is for one plate or multiple
        plates of the dish. Accordingly indicate number of plates as 1 or the
        desired quantity
      </p>
      <h1 className="text-xl text-gray-800 m-10 font-bold">
      ...
      <div className="m-10 w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
        <form className="space-y-6" action="#">
          ...
          <div className="mt-4">
            {ingredientSets.map((ingredientSet, setIndex) => (
              <div key={setIndex} className="flex space-x-2 mb-2">
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange(setIndex, "ingredient", e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange(setIndex, "quantity", e.target.value)
                  }
                  className="border p-2"
                />
              </div>
            ...
            <button
              onClick={addIngredientSet}
              className="bg-none border-2 border-[#272635] text-[#272635] rounded-lg px-4 py-2 mt-2 hover:bg-[#272635] hover:text-[#fff]"
            >
              Add Ingredients
            </button>
          </div>
          ...
          <div>
            <label
              ...
              className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
            >
              Enter the selling price for the dish
            </label>
            <input
              onChange={onChange}
              type="text"
          ...
          <button
            type="submit"
            onClick={handleClick}
            className="w-full font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-[#272635] text-[#fff] hover:bg-[#000]"
          >
            Add Dish
          </button>
        </form>
